...
#define MAXLEN	80		/* max length of strnigs */
...
void sort_arr(const char *p_array[]){
	...
	for(low_index=0;p_array[low_index] != 0 && p_array[low_index+1] != 0; low_index++){
		for(hi_index = low_index+1;p_array[hi_index] != 0; hi_index++){
			comp_val=strcmp(p_array[hi_index], p_array[low_index]);
			if (comp_val >= 0)
				continue;
			/* swap strings */
